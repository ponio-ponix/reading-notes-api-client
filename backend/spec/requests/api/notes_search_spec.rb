  ...
  describe "GET /api/books/:book_id/notes_search" do
    ...
    context "正常系" do
     it "returns 200 and notes/meta" do
         get "/api/books/#{book.id}/notes_search",
            params: { q: "ruby", page: 1, limit: 10 },
            headers: headers
      ...
      it "can filter by page range (page_from/page_to) when provided" do
        get "/api/books/#{book.id}/notes_search",
            params: { page_from: 2, page_to: 2, page: 1, limit: 10 },
            headers: headers
        expect(response).to have_http_status(:ok)
    ...
    context "404系" do
      it "returns 404 when book does not exist" do
        get "/api/books/999999/notes_search",
            params: { q: "ruby" },
            headers: headers
